...
def mesh_Disc(C=[0,0,0],r=1,N=[0,0,1],Fa=10, Fv=5): ##Hui new added
    # C is the center; N is the normal orthogonal to circle plane
    C = np.array(C)
    N = np.array(N)
    Fx = Fa; Fy = Fv+1
    ...
    Q[:,:,3] = M[1:,:M.shape[1]-1]
    Q = Q.reshape(((Fx)*(Fy-1),4),order='C')
    phi = np.linspace(0,2*np.pi,Fx)

    z = np.zeros(Fy)#np.linspace(0,h*sinH,Fy)
    ...
    if N[0]**2+N[1]**2==0:
        ...
        x = np.zeros(Fy)
        y = np.zeros(Fy)
        _, x = np.meshgrid(phi,x)
        _, y = np.meshgrid(phi,y)
    ...
    coo = np.linspace(0,r,Fy)
    A = (coo.repeat(Fx)).reshape(-1,Fx)

    Px = A*(U[0]*np.cos(phi)+V[0]*np.sin(phi)) + C[0] + x # A*np.cos(phi) + C[0]
    Py = A*(U[1]*np.cos(phi)+V[1]*np.sin(phi)) + C[1] + y # A*np.sin(phi) + C[1]
    Pz = A*(U[2]*np.cos(phi)+V[2]*np.sin(phi)) + C[2] + z # z
